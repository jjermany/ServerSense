...
- Keep Settings forms grouped by purpose with consistent control dimensions, full-width cards, responsive grids, active section navigation, and a visible retry path when initial configuration loading fails.
- Keep MFA Settings compact and responsive, with a visible enabled/off badge, guided enrollment steps, grouped QR/verification fields, and readable recovery-code tiles.
- Require the custom ServerSense request header on API mutations, keep cross-origin CORS disabled, bound request bodies, and never echo rejected credential input in validation errors.
- Keep login limits independent by account and source, bound and expire limiter keys, and serialize first-run account creation with a database writer lock after password hashing.
- Require an explicit installation secret; suppress HTTP client URL/protocol logs and SQL parameter values and keep log-handler registration idempotent.
- Bound synchronous external response bodies and reject redirects. Enforce compressed-response limits during decompression, before allocating the full expanded payload, for both synchronous responses and model streams. Bound incoming model stream bytes, events, text, and tool arguments; enforce the total prompt budget again after tool results and mark any truncated facts explicitly.
- Bound robust forecast estimation to 256 evenly spaced samples while retaining full-window sample count/coverage, and query only the relevant forecast window. Disk history must return the latest 500 samples in chronological order.
- Gather and normalize media responses before opening the persistence transaction; match only fetched external IDs when updating history.
- Match login usernames case-insensitively while preserving stored capitalization and case-sensitive password verification.
- Keep authenticator MFA optional and disabled for new and existing accounts until verified enrollment. Generate QR codes locally, encrypt active/pending secrets, expire pending enrollment, hash recovery codes, and consume TOTP/recovery codes atomically to reject replay. Require password reauthentication for MFA changes and a valid factor for disabling or replacing recovery codes. Issue no login session after password-only verification when MFA is enabled; revoke prior sessions on factor/recovery changes. Never expose MFA secrets through general user, Settings export, telemetry or SENSE responses.
...
- Keep the container health check on a fixed localhost URL using Python. Use the supported Alpine 3.24 runtime with current package updates, SMART tools and timezone data; remove temporary compiler dependencies and pip after installation, and patch pip before use. Verify native modules, migrations and packaged browser flows when changing the runtime base.
...
- Keep persistent application logging size-bounded through rotation. Rate-limit identical SMART-unavailable warnings per device while logging changed failures immediately and resetting suppression after recovery.
...
- Format user-facing UI and SENSE summary times with a 12-hour AM/PM clock in the configured timezone; retain UTC ISO timestamps for storage and transport. Supply explicit local-display fields beside raw tool timestamps and require interactive answers to use only the local-display form. Use local 12-hour times on 24-hour chart axes, dates on longer-range axes, and full local date/time/timezone hover labels. Format signed user-facing storage values by magnitude with human-readable units; never expose raw byte counts or contradictory signs/arrows in trend labels.
- Keep the 24-hour storage chart limited to measured history; never append forecast points or show projection legends in that view. Longer-range views retain deterministic projections.
...
- Treat every Sonarr/Radarr installation as a separately named integration. Encrypt API keys, poll only fixed read-only v3 endpoints without redirects, discard raw payloads and paths, deduplicate history and replace bounded calendar snapshots per integration, and expose only normalized media activity to SENSE while AI is enabled. Use Sonarr episode `airDateUtc` and prefer Radarr's provider-selected calendar `releaseDate`. Confirm a quality upgrade only from a provider deletion whose explicit reason is `Upgrade`; never infer one from a grab, import, rename, or quality difference alone. For both Sonarr and Radarr, correlate bounded persisted history one-to-one within the same integration and stable movie or episode identity: use an ordered grab → `Upgrade` deletion → import chain when a matching grab is available, and retain nearby `Upgrade` deletion/import pairing when it is not. Load correlation companions before applying reporting-period and event filters so sequences split across polls and import-only lists remain classified, while returning only events in the requested period. Represent each paired replacement as one normalized upgrade item in general activity lists, calculate logical upgrade net bytes only when both provider-reported file sizes are present, and describe calendar entries as upcoming/eligible rather than guaranteed downloads.
- Update `README.md`, this file, and `docs/IMPLEMENTATION_CHECKLIST.md` as behavior changes.
- Keep Docker publishing cache exports optional (`ignore-error=true`) with an explicit GitHub Actions cache v2 scope; never suppress application verification, image build or registry push errors to work around a cache-service failure.
