...
- Format user-facing UI and SENSE summary times with a 12-hour AM/PM clock in the configured timezone; retain UTC ISO timestamps for storage and transport. Supply explicit local-display fields beside raw tool timestamps and require interactive answers to use only the local-display form.
