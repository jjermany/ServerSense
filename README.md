...
Live CPU, memory, and network telemetry is collected every 30 seconds by default, then temporarily every 5 seconds while an authenticated ServerSense UI is visible. The visible UI renews a process-local 45-second activity lease; hidden or closed tabs stop renewing it and collection automatically returns to the background cadence. Docker is collected every 15 seconds while active and every 30 seconds otherwise. The Overview and Docker pages refresh every 5 seconds while visible and refresh immediately when the browser tab becomes visible again. SMART inventory remains independent at a conservative 15-minute cadence to avoid excessive hardware polling; storage history remains hourly. These intervals can be overridden with the corresponding `SERVERSENSE_*_INTERVAL_SECONDS` environment variables shown in `.env.example`.

Authenticated API responses are explicitly non-cacheable. The Overview displays the freshest page telemetry time, and each deterministic or cached SENSE insight displays its own measurement or generation time in the configured timezone. The Docker view also displays its latest sample time. This makes stale cached summaries, older alert explanations, unchanged measurements, and stalled collectors distinguishable from current telemetry.
...
Configure the endpoint without `/v1`, model name, optional API key, timeout, temperature, tool-call limit, and response-token limit. API keys are encrypted at rest using a key derived from `SERVERSENSE_SECRET_KEY`. Keep that secret stable across upgrades. Provider responses stream into the chat UI and can be stopped without persisting a partial exchange. SENSE sends at most the six most recent messages from an active conversation, treats that history as context without repeating earlier answers in follow-ups, and drops context after 30 minutes of inactivity. Current UTC time and current-turn grounding instructions are supplied to the model; clear upcoming-media questions and accepted offers to list calendar titles require the calendar tool, and exact duplicate tool calls are reused. Conversations can be removed from the Ask SENSE sidebar and otherwise expire after 30 days.
