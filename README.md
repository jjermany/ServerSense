...
Container output omits successful HTTP access entries, including UI polling and health checks, to keep routine logs quiet. Failed HTTP requests and application warnings/errors remain visible; `SERVERSENSE_LOG_LEVEL` controls application log verbosity. The persistent `/config/logs/serversense.log` rotates at 5 MB and retains five backups, bounding the active log set to approximately 30 MB. Identical SMART-unavailable warnings are emitted once and then at most once every six hours per device; a changed failure is logged immediately, and a successful read resets suppression.
...
Combined array capacity is calculated from the assigned Unraid data-disk filesystem totals in read-only runtime metadata. Parity, boot, individual-device totals, and named pools such as cache are excluded; pools are reported separately. ServerSense never falls back to the container root filesystem when capacity metadata is unavailable. Forecasts, history, alerts, dashboards, and SENSE use only samples from the newest compatible measurement source, so older `/mnt/user` or fallback samples cannot be mixed into normalized array trends. Signed dashboard trend rates use human-readable decimal units, such as `-846.7 GB/day`, instead of raw bytes. The 24-hour storage chart shows measured history only; future projections appear only on longer-range views. The 24-hour storage chart labels its horizontal axis with configured-timezone 12-hour times; longer ranges use dates, and hover labels include the full local date, 12-hour time, and timezone.
