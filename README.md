...
Combined array capacity is calculated from the assigned Unraid data-disk filesystem totals in read-only runtime metadata. Parity, boot, individual-device totals, and named pools such as cache are excluded; pools are reported separately. ServerSense never falls back to the container root filesystem when capacity metadata is unavailable. Forecasts, history, alerts, dashboards, and SENSE use only samples from the newest compatible measurement source, so older `/mnt/user` or fallback samples cannot be mixed into normalized array trends. Signed dashboard trend rates use human-readable decimal units, such as `-846.7 GB/day`, instead of raw bytes.
