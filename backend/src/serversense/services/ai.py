...
GROUNDED_ANSWER_PROMPT = """Answer the current request now. Do not repeat any earlier answer or tool-call preamble. For array-wide storage, use only values explicitly scoped as combined_array_data_disks; never substitute individual disk or named-pool capacity. For media titles, episode numbers, and dates, use only values present in this turn's tool results. Honor confirmed_quality_upgrade; unclassified_import means unknown, never regular, new, or non-upgrade. For upgrade storage, report only supplied net_bytes_change or known_net_bytes_change, identify missing-size exclusions, and call it logical file-size change rather than measured array growth. Present times only from supplied local-display values with AM or PM and the configured timezone; never show raw UTC timestamps. If requested items are absent, say none were returned."""
...
def _required_tool(question: str, history: Sequence[dict[str, str]]) -> str | None:
    """Require evidence tools for clear media-history and upcoming requests."""
    ...
    if (
        "quality upgrade" in current
        or "quality upgraded" in current
        or (
            any(term in current for term in ("upgrade", "upgraded"))
            and any(term in f"{current} {history_text}" for term in media_terms)
        )
    ):
    ...
    if any(phrase in current for phrase in activity_phrases):
        return "get_media_activity_items"
    ...
    if acceptance:
        ...
        if any(term in previous_assistant for term in ("upcoming", "calendar", "air date")):
            return "get_upcoming_media"
        if any(term in previous_assistant for term in ("upgrade", "upgraded")):
            return "get_quality_upgrades"
        if any(term in previous_assistant for term in ("import", "download", "grab")):
            return "get_media_activity_items"
    return None
